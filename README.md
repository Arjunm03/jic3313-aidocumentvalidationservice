...
Using the terminal, navigate to an appropriate folder to store this project. Then, in the terminal, type the following command:
```
git clone https://github.com/Arjunm03/jic3313-aidocumentvalidationservice.git
```
The project should then be downloaded into local storage at the specified directory.
...
## Build Instructions:
No building is needed for this project except for the exceptional circumstances below. After downloading the application repository, proceed to the **"Installation of Application"** section to continue setting the application up.

_Exceptional Circumstances:_ Should there be a need to re-train the custom classification model for the SF-86 form with the substitute fake dataset (as we have done for the implementation so far), navigate to the data folder using a terminal and run the following commands:
```
python mimedata.py
python fillingfields.py
```
This will create a number of correctly filled SF-86 forms with random person data that can be used to re-train the Document Intelligence Studio model. The number of PDFs generated can be altered by changing the "num_rows" variable in the mimedata.py file. These PDFs will need to be re-uploaded to the 'correct' folder on the Azure storage being used. The 'incorrect' PDF dataset can be generated as seen fit by corrupting or removing elements/fields in copies of the correct PDFs.
These circumstances are unlikely, though, since we expect the client to use the existing fake dataset we have generated, or use real SF-86 forms that have been accepted/rejected in the past as more accurate training data.

## Installation of Application:
To install this application, use the following steps:

- Navigate to the server folder.
- Run the command "npm install" to install all dependencies in the server folder:
  ```
  npm install
  ```
- Navigate to the client folder.
- Run the command "npm install" to install all dependencies in the client folder:
  ```
  npm install
  ```
...
## Version 1.0.0
### Features:
- Users can be created (Admins cannot be created, as intended)
- Users can upload documents
- Users can specify the document type
- Admins can run AI Validation on document
- Users can see the output of AI Validation
- Admins can override AI Validation
- Users can filter documents based on type or status
- Users can delete documents
- Users can preview documents before or after they are processed
- Admins have all functionality that users do but can also view every document the organization has uploaded and can run or override the AI Validation
...
- Validation model will sometimes hallucinate and return unreasonable conclusions

### Bugfixes:
- Document type is now preserved in the database, as intended
- Specifying document type no longer throws an error
- Validation model hallucinates far less frequently and assigns low confidence probabilities when it does hallucinate
